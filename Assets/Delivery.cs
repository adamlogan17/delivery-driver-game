...
public class Delivery : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        ...
        if(other.tag == "Package" && !hasPackage) {
            ...
            GameObject customer = GameObject.FindGameObjectsWithTag("Customer")[0]; // gets first object with the tag Customer (there should only be 1)
            spriteRenderer.color = collisionObject.color;
            customer.GetComponent<SpriteRenderer>().color = spriteRenderer.color;
        ...
        if(other.tag == "Customer" && hasPackage) {
            Debug.Log("Drived over customer!");
            hasPackage = false;
    ...
    private void putFrontHouse(Collider2D objectToMove) {
